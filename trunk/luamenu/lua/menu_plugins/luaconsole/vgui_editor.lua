/******************************************************************************\
  ...
  Edited by LuaStoned (the-stone/stoned) to fit Lua syntax
\******************************************************************************/

local EDITOR = {}

surface.CreateFont("Courier New", 16, 400, false, false, "LuaEditor")
surface.CreateFont("Courier New", 16, 700, false, false, "LuaEditorBold")

function EDITOR:Init()
	self:SetCursor("beam")
	
	surface.SetFont("LuaEditor")
	...
	self.Color = {}
	self.Color.Focus = Color(220,220,220,255)
	self.Color.Cursor = Color(72,61,139,255)
	self.Color.Selection = Color(170,170,170,255)
	self.Color.Background = Color(200,200,200,255)
	self.Color.Foreground = Color(230,230,230,255)
	...
	elseif code == MOUSE_RIGHT then
		...
		if self:HasSelection() then
			...
			menu:AddOption("Search", function()
				LuaMenu.Panel.Search:Search(self:GetSelection())
				LuaMenu:SetTab(LuaMenu.Panel.Search)
			end)
		end
...
function EDITOR:PaintLine(row)
	...
	if row == self.Caret[1] and self.TextEntry:HasFocus() then
		...
		draw.RoundedBox(0,width * 3 + 5, (row - self.Scroll[1]) * height, self:GetWide() - (width * 3 + 5), height,self.Color.Focus)
	end
	...
	if self:HasSelection() then
		...
		if row == line and line == endline then
			...
			draw.RoundedBox(0,char * width + width * 3 + 6, (row - self.Scroll[1]) * height, width * (endchar - char), height,self.Color.Selection)
		elseif row == line then
			--surface.DrawRect(char * width + width * 3 + 6, (row - self.Scroll[1]) * height, width * (length - char + 1), height)
			draw.RoundedBox(0,char * width + width * 3 + 6, (row - self.Scroll[1]) * height, width * (length - char + 1), height,self.Color.Selection)
		elseif row == endline then
			--surface.DrawRect(width * 3 + 6, (row - self.Scroll[1]) * height, width * endchar, height)
			draw.RoundedBox(0,width * 3 + 6, (row - self.Scroll[1]) * height, width * endchar, height,self.Color.Selection)
		elseif row > line and row < endline then
			--surface.DrawRect(width * 3 + 6, (row - self.Scroll[1]) * height, width * (length + 1), height)
			draw.RoundedBox(0,width * 3 + 6, (row - self.Scroll[1]) * height, width * (length + 1), height,self.Color.Selection)
		end
	end
	
	draw.SimpleText(tostring(row), "LuaEditor", width * 3 - 22, (row - self.Scroll[1]) * height, Color(128, 128, 128, 255),TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER) -- align right broken? - 22
	...
	for i,cell in ipairs(self.PaintRows[row]) do
		if offset < 0 then
			if cell[1]:len() > -offset then
				...
				if cell[2][2] then
					draw.SimpleText(line .. " ", "LuaEditorBold", width * 3 + 6, (row - self.Scroll[1]) * height, cell[2][1])
				else
					draw.SimpleText(line .. " ", "LuaEditor", width * 3 + 6, (row - self.Scroll[1]) * height, cell[2][1])
				end
			else
			...
			if cell[2][2] then
				draw.SimpleText(cell[1] .. " ", "LuaEditorBold", offset * width + width * 3 + 6, (row - self.Scroll[1]) * height, cell[2][1])
			else
				draw.SimpleText(cell[1] .. " ", "LuaEditor", offset * width + width * 3 + 6, (row - self.Scroll[1]) * height, cell[2][1])
			end
...
function EDITOR:PaintTextOverlay()
	...
	if self.TextEntry:HasFocus() and self.Caret[2] - self.Scroll[2] >= 0 then
		...
		if (CurTime() - self.Blink) % 0.8 < 0.4 then
			...
			draw.RoundedBox(0,(self.Caret[2] - self.Scroll[2]) * width + width * 3 + 6, (self.Caret[1] - self.Scroll[1]) * height, 1, height,self.Color.Cursor)
		end
...
function EDITOR:Paint()
	...
	draw.RoundedBox(0,0,0,self.FontWidth * 3 + 4, self:GetTall(),self.Color.Background)
	
	--surface.SetDrawColor(0, 0, 0, 255)
	--surface.DrawRect(0, 0, self.FontWidth * 3 + 4, self:GetTall())
	
	draw.RoundedBox(0,self.FontWidth * 3 + 5, 0, self:GetWide() - (self.FontWidth * 3 + 5), self:GetTall(),self.Color.Foreground)
...
function EDITOR:_OnTextChanged()
	...
	if not ctrlv then
		if text == "\n" then return end
		if text == "}" and LuaMenu.Settings.EditorAutoindent == true then
...
function EDITOR:CommentSelection(shift) -- Multi-line comment feature ((shift-)ctrl-k) (idea by Jeremydeath)
	...
	local comment_char = self:GetParent().Lua and "--" or "//"
...
function EDITOR:_OnKeyCodeTyped(code)
	...
	if control then
		...
		if code == KEY_ENTER then
			...
			if LuaMenu.Settings.EditorAutoindent == true and (string.match("{" .. row .. "}", "^%b{}.*$") == nil) then tabs = tabs .. "    " end
			self:SetSelection("\n" .. tabs)
		elseif code == KEY_UP then
...
do -- E2 Syntax highlighting
	local function istype(tp)
		return true--return wire_expression_types[tp:upper()] or tp == "number"
	end
	...
	local function CheckGlobal(func)
		--if LuaMenu._G[func] ~= nil then return LuaMenu._G[func] end
		if _E[func] ~= nil then return _E[func] end
		if _G[func] ~= nil then return _G[func] end
	...
	function EDITOR:SyntaxColorLine(row)
		...
		// code from prev versions of luapad, thanks to darksunrise
		
		while self.character do
			local tokenname = ""
			self.tokendata = ""
			
			while self.character and self.character == " " do self:NextCharacter() end
			if(!self.character) then break end
			
			if(self.character >= "0" and self.character <= "9") then
				while self.character and (self.character >= "0" and self.character <= "9" or self.character == "." or self.character == "_") do self:NextCharacter() end
				
				tokenname = "number"
			elseif(self.character >= "a" and self.character <= "z" or self.character >= "A" and self.character <= "Z") then
				...
				elseif(CheckGlobal(sstr) && (type(CheckGlobal(sstr)) == "function" or CheckGlobal(sstr) == "f"
				or CheckGlobal(sstr) == "e" or CheckGlobal(sstr) == "m" or type(CheckGlobal(sstr)) == "table")
				or (lasttable && lasttable[sstr])) then -- Could be better code, but what the hell; it works
					
					if(type(CheckGlobal(sstr)) == "table") then
						lasttable = CheckGlobal(sstr);
					end
					
					if((CheckGlobal(sstr) == "e" or _E[sstr]) && sstr == string.upper(sstr)) then
						tokenname = "enumeration";
			...
			elseif(self.character == "\"") then -- TODO: Fix multiline strings, and add support for [[stuff]]!
			...
			elseif(self.character == "'") then
				...
				tokenname = "string2"
			elseif(self.character == "/" or self.character == "-") then -- TODO: Multiline comments!
			
				local lastchar = self.character;
				self:NextCharacter()
				
				if(self.character == lastchar) then
					while self.character do
						self:NextCharacter()
					end					
					tokenname = "comment"
...
vgui.Register("LuaEditor", EDITOR, "Panel");
